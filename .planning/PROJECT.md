...
- [x] Validated in Phase 2: Fixtures & types (2026-09-17) — every authored observation cites a real fixture record (a fact or a deviation id), carries the record’s own sentence in a comment that a build check compares against the live record, and was read against that record by a named person: Matthew Koeberg confirmed all twelve rows on 2026-09-17 in `docs/analysis/provenance-check.md` (FR-21a). Laid alongside it: the closed sets defined once in `lib/data/types.ts` with the grade meaning *the record states this* unconstructible on an authored observation, the plant subset copied byte-faithful from ipv-demo at `8fd097a`, the server-only asset register with a two-layer isolation rule in `verify`, and `FIXTURE_VERSION` pinned to a content hash. Evidence: `02-VERIFICATION.md` (4/4), `docs/analysis/provenance-check.md`.
...
---
*Last updated: 2026-09-17 after Phase 2 completion (execute-phase)*
