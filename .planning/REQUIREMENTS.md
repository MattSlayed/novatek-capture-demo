...
- [x] **REQ-FR-2**: A client can ask the server which account it is acting as and receives the account or 401. *[Verified by: test]*
- [x] **REQ-FR-3**: An artisan can end the session; the cookie is cleared. Stated limitation (Limits screen + handover): the credential is stateless, so a copied credential remains valid until expiry; no revocation is claimed. *[Verified by: test]*
...
- [x] **REQ-FR-5**: An artisan can open a work order assigned to them and receive the order, its assets, its clock, and any verifications, proposals and decisions already recorded; server-only fields stripped. *[Verified by: test]*
...
- [x] **REQ-FR-15**: For a photograph only a hash of the full image, its size, its type and a bounded thumbnail leave the device; the request body contains no full-resolution image field; the thumbnail bound is enforced client- and server-side in encoded bytes. *[Verified by: test + inspection]*
- [x] **REQ-FR-16**: A spoken note's request carries a hash, a size, a type and a duration and no audio — exactly the four declared fields; the governed sentence states exactly this and no more. *[Verified by: test + inspection]*
...
| REQ-FR-2 | Phase 3 | Complete |
| REQ-FR-3 | Phase 3 | Complete |
| REQ-FR-4 | Phase 3 | Complete |
| REQ-FR-5 | Phase 3 | Complete |
...
| REQ-FR-24 | Phase 3 | Complete |
| REQ-FR-27 | Phase 3 | Complete |
| REQ-FR-57 | Phase 3 | Complete |
| REQ-FR-61 | Phase 3 | Complete |
