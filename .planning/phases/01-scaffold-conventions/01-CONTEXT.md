...
- **D-02:** The sibling `../ipv-demo` is the reference for `tsconfig.json`, `eslint.config.mjs` (`defineConfig` + `eslint-config-next/core-web-vitals` + `/typescript`, with the `globalIgnores` block), `app/layout.tsx` structure and `app/globals.css` import order — copy the patterns, not its 3D dependencies (`three`, `@react-three/*`, `@sparkjsdev/spark`, `motion`, `postprocessing` are not installed). Source shape follows the seed's cold start: `app/`, `components/`, `lib/`, `public/`, `scripts/`; the code owns the final layout.
...
- **D-22:** *[research question, resolved in planning]* AD-15 requires that deployment runs the same command as a developer. The mechanism is undecided pending research: either Vercel's build command is `npm run verify` (which requires the Playwright/axe step to run inside Vercel's build image), or `verify` runs in a GitHub Actions workflow on every push with the Vercel deployment gated on that check, or the axe step runs in both places with browser installation handled per environment. The plan must state which, why, and how a failing check stops the deployment; it must not weaken `verify` to make deployment pass.
