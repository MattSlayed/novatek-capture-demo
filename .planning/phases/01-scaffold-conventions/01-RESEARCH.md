...
1. **Does installed `axe-core@4.13.0` actually enable WCAG 2.2 A/AA rules by default?** Deque's indexed docs say disabled-pending-adoption; WCAG 2.2 has been a Recommendation since Oct 2023. Recommendation: Wave 0 runs `node -e "console.log(require('axe-core').getRules(['wcag22aa']))"` against the real installed version before writing the WCAG-check script's `runOnly` tags.
2. **Is Vercel's build container genuinely unable to run `playwright install --with-deps`?** No official doc confirms/denies this for the *build* step (most material discusses the 50MB *function* bundle-size limit, a different constraint). Recommendation: default to the GitHub Actions + Deployment Checks split; spend one build-log-reading cycle to falsify before removing it.
