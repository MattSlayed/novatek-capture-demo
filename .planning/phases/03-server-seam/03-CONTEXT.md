...
- `app/api/**` — every route the seed's API contracts name except `POST /api/referrals` (P9): `session` (POST/GET/DELETE), `orders`, `orders/[id]`, `orders/[id]/open`, `orders/[id]/close`, `verify`, `captures`, `decisions`, `sync`, `hours` (GET only; POST → 405), `walk/[orderId]`, `health` (GET/HEAD). Every handler: params awaited, cookie read in the handler, nothing identity-bearing read from a body. *Amended 2026-09-17 from research (03-RESEARCH.md Pitfall 1, Pitfall 2, Code Example 4):* the seed's `export const runtime = "nodejs"` / `export const dynamic = "force-dynamic"` are a hard `next build` error under the locked `cacheComponents: true` (Phase 1 D-11; reproduced against this repository), so no route exports either. A route is dynamic by construction once it reads the session cookie or resolves `[id]`, which every route here does except `/api/health`; that route calls `await connection()` from `next/server` as its first statement so `boot_id` and `uptime_s` are never frozen at build time (curl H). `POST /api/hours` exports an explicit handler that returns 405 through the responder, because Next's auto-generated 405 carries none of the universal headers (NFR-F1).
