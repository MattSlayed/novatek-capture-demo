...
```
BMAD (/bmad:master → bmad-help), output folder = docs/       GSD
──────────────────────────────────────────────────────       ─────────────────────────────────────────
bmad-product-brief    → docs/briefs/brief-<name>-<date>/brief.md
bmad-prd              → docs/prds/prd-<name>-<date>/prd.md   /gsd-ingest-docs docs/ --mode new --manifest docs/ingest.yaml
bmad-create-ux-design → docs/ux-design-specification.md  ─►    manifest types: architecture.md=ADR · ux=SPEC · prd=PRD · brief=DOC
bmad-create-architecture → docs/architecture.md                → .planning/{PROJECT,REQUIREMENTS,ROADMAP,STATE}.md
...
3. **BMAD**: inside the new repo run `npx bmad-method@latest install` interactively (the user runs it via `!`). Choose: tool **claude-code**; modules **bmm** (+ **cis** optional); output folder **`docs`** (so the skills write where GSD ingests); skills target **global `~/.claude/skills`** (the installer offers project-relative `.claude/skills` too — choosing that would leave the stale 2026-05-19 global set live everywhere else). The headless equivalent is `npx bmad-method@latest install --yes --modules bmm,cis --tools claude-code --output-folder docs`, but it may not surface the skills-target prompt — use it only after one interactive run confirms the choices. Expect: `_bmad/_config/manifest.yaml` with `version: "6.11.0"` (the `_cfg` path is the alpha layout). Verify: `grep version _bmad/_config/manifest.yaml` shows 6.11.0; `ls -la ~/.claude/skills/bmad-help/SKILL.md` is newer than 2026-05-19; `python3 _bmad/scripts/resolve_customization.py --help` (or the call a skill makes) runs; `/bmad:master` orients without a "catalog not found" warning. Commit `_bmad/` (it is the project's BMAD state) but gitignore `_bmad/**/*.user.*` and any `config.user.yaml` — personal settings, not project state. Committing `_bmad/` is a project choice, not a BMAD requirement.
