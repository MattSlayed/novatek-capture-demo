...
import { THUMB_MAX_ENCODED_BYTES, MIME_MAX_CHARS, NOTE_MAX_CHARS } from "../limits/index.ts";
import {
  isUuidShaped,
  isSha256Hex,
  isIsoUtcZ,
...
test("validateCapturePayload refuses a mime longer than MIME_MAX_CHARS, parameter and all", () => {
  // The allowlist matches on the type/subtype before any `;`, so a
  // value that starts with an allowed base used to pass however long
  // the parameter after it was, and was then stored verbatim.
  const padded = `image/jpeg;${"x".repeat(MIME_MAX_CHARS)}`;
  const refusal = validateCapturePayload({ ...VALID_PHOTO_PAYLOAD, mime: padded });
  assert.ok(refusal, "an over-long mime must be refused even with an allowed base type");
  assert.equal(refusal.code, "bad_shape");
  assert.equal(refusal.field, "mime");

  // The real values this project emits are nowhere near the ceiling.
  assert.equal(validateCapturePayload({ ...VALID_PHOTO_PAYLOAD, mime: "image/jpeg" }), null);
  assert.equal(validateCapturePayload({ ...VALID_VOICE_PAYLOAD, mime: "audio/webm;codecs=opus" }), null);
});
...
test("validateDecisionPayload refuses a note longer than NOTE_MAX_CHARS", () => {
  const refusal = validateDecisionPayload({
    ...VALID_DECISION_PAYLOAD,
    note: "x".repeat(NOTE_MAX_CHARS + 1),
  });
  assert.ok(refusal);
  assert.equal(refusal.code, "media_too_large");
  assert.equal(refusal.field, "note");

  // A note exactly at the ceiling is still accepted.
  assert.equal(
    validateDecisionPayload({ ...VALID_DECISION_PAYLOAD, note: "x".repeat(NOTE_MAX_CHARS) }),
    null,
  );
});
...
const ENVELOPE = { order_id: "wo-0142", schema_version: 1 };

test("idempotencyHash ignores key order and an unenumerated field, but changes when an enumerated value changes", () => {
  const a = idempotencyHash("capture", { sha256: VALID_SHA256, bytes: 1, mime: "image/jpeg" }, ENVELOPE);
  const b = idempotencyHash("capture", { mime: "image/jpeg", bytes: 1, sha256: VALID_SHA256 }, ENVELOPE);
  assert.equal(a, b);

  const withStrayField = idempotencyHash(
    "capture",
    {
      sha256: VALID_SHA256,
      bytes: 1,
  ...
  const changedValue = idempotencyHash(
    "capture",
    { sha256: VALID_SHA256, bytes: 2, mime: "image/jpeg" },
    ENVELOPE,
  );
  assert.notEqual(a, changedValue);
});

test("idempotencyHash changes when the envelope's kind, order_id or schema_version changes", () => {
  // The projected payload is identical in every pair below; only the
  // envelope differs. A `seen` entry answers "is this the same item?",
  // so an envelope change must be a different digest — otherwise a
  // client_id reused across kinds or orders replays the other item's
  // stored result as a duplicate.
  const openHash = idempotencyHash("order_open", {}, ENVELOPE);
  const closeHash = idempotencyHash("order_close", {}, ENVELOPE);
  assert.notEqual(openHash, closeHash);

  const otherOrderHash = idempotencyHash("order_open", {}, { ...ENVELOPE, order_id: "wo-0151" });
  assert.notEqual(openHash, otherOrderHash);

  const otherVersionHash = idempotencyHash("order_open", {}, { ...ENVELOPE, schema_version: 2 });
  assert.notEqual(openHash, otherVersionHash);
});

test("idempotencyHash is a pure function of (kind, envelope, projected payload)", () => {
  const a = idempotencyHash("order_open", { device_claimed_opened_at: VALID_ISO }, ENVELOPE);
  const b = idempotencyHash("order_open", { device_claimed_opened_at: VALID_ISO }, ENVELOPE);
  assert.equal(a, b);
  assert.equal(typeof a, "string");
  assert.equal(a.length, 64); // sha256 hex digest
});

test("idempotencyHash never throws on an envelope whose fields have not been proved yet", () => {
  // AD-1 runs idempotency before shape, so this function is reached by
  // items whose order_id may be absent and whose schema_version may be
  // any JSON value at all.
  const absent = idempotencyHash("order_open", {}, { order_id: undefined, schema_version: undefined });
  assert.equal(absent.length, 64);
  assert.equal(absent, idempotencyHash("order_open", {}, { order_id: "", schema_version: null }));
});
